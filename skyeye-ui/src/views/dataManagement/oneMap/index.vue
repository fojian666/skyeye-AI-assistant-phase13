  ...
  <div class="se-container">
    <div class="left-content-map" v-show="showTree" style="height: 100%">
      <div class="title">
        ...
        <el-button type="text" style="float: right" class="iconfont icon-shouqi"
                   @click="showTree = !showTree"></el-button>
      </div>
      <div class="filter">
    ...
    <div class="left-content-map" v-show="!showTree" style="height: 46px">
      <div class="title">
        <span class="tab-title">一张图</span>
        <el-button type="text" style="float: right" class="iconfont icon-zhankai"
                   @click="showTree = !showTree"></el-button>
      </div>
    </div>
  ...
  methods: {
    ...
    renderContent(h, { node }) {
      ...
      return (
        <span class="custom-tree-node" style="display:flex;align-items:center">
          {node.isLeaf
            ? [
              <i key="icon" class="el-icon-document"
                 style="margin-right:8px;padding-left:20px;color: #177DE4;"></i>,
              <span
                key="label"
                title={node.label}
    ...
    getDefaultCheckedNodes() {
      ...
      if (lowAltitude && lowAltitude.children) {
        const panorama = lowAltitude.children.find(
          (item) => item.data_type === 'panorama' || item.label === '全景点'
        );
        const panoramaCoverage = lowAltitude.children.find(
          (item) =>
            item.data_type === 'panorama_coverage' ||
            (item.label && item.label.includes('全景覆盖'))
        );
    ...
    removeThreeDDataGroup() {
      ...
      const groupLabels = new Set(
        (this.data[groupIndex].children || []).map((c) => c.label)
      );
      ...
      if (this.$refs.tree) {
        const checked = this.$refs.tree
          ...
          .filter((n) => !groupLabels.has(n.label));
    ...
    async fetchAndInsert3dtilesGroup() {
      try {
        ...
        const dirChildren = (this._oneMap3DDirChildren || []).filter(
          (n) => n.data_type !== '3dtiles'
        );
        const usedLabels = new Set(dirChildren.map((n) => n.label));
        const apiChildren = (res.data || []).map((item) =>
          map3dtilesItemToTreeNode(item, usedLabels)
        );
        const merged = dedupeByLabel([...dirChildren, ...apiChildren]).map((node) =>
          node.service
            ? { ...node, service: normalizeStaticResourceUrl(node.service) }
            : node
        );
    ...
    async fetchOneMapBundle(para, options = {}) {
      ...
      try {
        ...
        this._oneMap3DDirChildren =
          oneMap3DGroup && oneMap3DGroup.children ? oneMap3DGroup.children.slice() : [];
        ...
        const targetIndex = this.data[0].children.findIndex(
          (item) => item.source_type === '影像服务'
        );
        if (targetIndex !== -1) {
