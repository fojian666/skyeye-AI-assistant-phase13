<template>
    <div class="map-2d-panel">
        ...
        <div class="toolbar" :style="targetDivStyle">
            ...
            <span style="color: #cccccc">|</span>
            <div @click="clearDraw"><span class="icon iconfont icon-qingchu icon-toolbar"></span><span>清除</span></div>
        </div>
        ...
        <div class="view-single" v-if="isShowSingleDiv">
            <viewSingle
                ...
                @closeDiv="
                    ...
                    singleObj = null;
                "
                @skipMulti="skipMulti"></viewSingle>
        </div>
...
import { getClueData, getPanoramaImageApi, getTimeAxisDataApi } from '@/api/commonApi';
...
export default {
    ...
    methods: {
        ...
        async syncLayers(checkedLeafNodes) {
            ...
            const checkedLabels = new Set(checkedLeafNodes.map((node) => this.getLayerKey(node)).filter((key) => key != null));
        ...
        drawRegion(polygon, name, lat, lng, subRegions) {
            if (!this.map) return;
            // 清除上一个区域
            if (this._regionLayers) {
                this._regionLayers.forEach((l) => this.map.removeLayer(l));
                this._regionLayers = null;
            }
            ...
            if (subRegions && subRegions.length) {
                subRegions.forEach((region) => {
                    ...
                    const layer = L.polygon(rRings, {
                        color: region.color || '#f56c6c',
                        weight: 2,
                        fillColor: region.color || '#f56c6c',
                        fillOpacity: 0.2
                    }).addTo(this.map);
        ...
        initLegacyMap() {
            ...
            const mapMaxZoom = this._crsMaxZoom != null ? Math.min(this.maxZoom, this._crsMaxZoom) : this.maxZoom;
                ...
                if (zoom >= 14) {
                    if (!isShowingImagery) {
                        layerGroup.eachLayer((marker) => {
                            ...
                            const bboxBounds = L.latLngBounds(L.latLng(bbox[1], bbox[0]), L.latLng(bbox[3], bbox[2]));
        ...
        getPanoramaPointData(node) {
            ...
            if (this.panoramaPointList.length > 0) {
                if (node) {
                    node.center = JSON.stringify(toMapLatLng(this.panoramaPointList[0].latitude, this.panoramaPointList[0].longitude));
                }
                            ...
                            circle
                                .bindPopup(
                                    '<div style="font-family: Arial; font-size: 14px; color: #555;">' +
                                        '<strong style="color: black;font-weight: bold;font-size: 19px">点位名称: ' +
                                        item.pointName +
                                        '</strong><br>' +
                                        '<strong style="color: black;font-weight: bold;font-size: 19px">批次数量:' +
                                        item.panoramaImageCount +
                                        '</strong> <br>' +
                                        '<strong style="color: black;font-weight: bold;font-size: 19px">最近拍摄时间:' +
                                        item.latestTime +
                                        '</strong> <br>' +
                                        '<strong style="color: black;font-weight: bold;font-size: 19px">飞行员信息:' +
                                        item.gridOperator +
                                        '</strong>' +
                                        '</div>'
                                )
        ...
        getTempPointData(node) {
            ...
            if (this.tempPointList.length > 0) {
                if (node) {
                    node.center = JSON.stringify(toMapLatLng(this.tempPointList[0].latitude, this.tempPointList[0].longitude));
                }
