<template>
    <div class="map-3d-panel">
        <div id="cesiumContainer" :class="{ 'cesium-split-left': isShowSingleDiv }" style="height: 100%"></div>
        <SceneLayerManager ref="sceneLayerManager" map-id="cesiumContainer" :load-resources="false" />
        <div v-show="cesiumTooltipVisible" class="cesium-tooltip" :style="cesiumTooltipStyle" v-html="cesiumTooltipContent"></div>
        <div class="detectlist" v-if="activePanoramaPoint && !isShowSingleDiv">
            <div class="title">
            ...
            <el-table :data="listData" style="background-color: white">
                ...
                <el-table-column property="operation" label="操作">
                    <template slot-scope="scope">
                        <el-button type="text" size="small" @click="openSingleView3D(scope.row)">查看 </el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        <div class="view-single view-single-3d" v-if="isShowSingleDiv">
        ...
        initialCenter: {
            handler(val) {
                if (Array.isArray(val) && val.length === 2 && this.viewer && this.$refs.sceneLayerManager) {
                    this.$refs.sceneLayerManager.flyToView({ lat: val[0], lng: val[1] }, this.initialZoom);
                }
            },
            deep: true
        },
    ...
    methods: {
        ...
        flyToHome() {
            if (this.$refs.sceneLayerManager) {
                this.$refs.sceneLayerManager.flyToView({ lat: this.initialCenter[0], lng: this.initialCenter[1] }, this.initialZoom);
                return;
            }
            if (this.viewer && this.Cesium) {
        ...
        refreshPanoramaIfNeeded() {
            const hasPanoramaLayer = this.currentLeafNodes.some((n) => n.data_type === 'panorama' || n.data_type === 'panorama_coverage');
            if (hasPanoramaLayer) {
        ...
        refreshTempPanoramaIfNeeded() {
            const hasLayer = this.currentLeafNodes.some((n) => n.data_type === 'temp_panorama' || n.data_type === 'temp_panorama_coverage');
            if (hasLayer) {
        ...
        async addLayers3D(addLeafNodes) {
            ...
            const loadableNodes = addLeafNodes.filter((node) => CESIUM_TREE_LAYER_TYPES.includes(node.data_type));
        ...
        findCesiumImageryNode() {
            ...
            const imageryNodes = children.filter((item) => item.source_type === '影像服务' && String(item.gis_service_type) !== '3' && item.service);
            if (imageryNodes.length === 0) return null;
            const defaultNode = imageryNodes.find((item) => item.isShow === 1 || item.isShow === '1');
        ...
        resolveCesiumBasemapSource() {
            ...
            if (this.baseMapService && cfgType === '3') {
                console.warn('三维底图：config 为 MBTiles(type=3)，仅用于二维；请确保「基础地理数据」存在 gis_service_type≠3 的影像服务节点');
            }
            return null;
        },
        ...
        flyToRegion3D(lat, lng, polygon) {
            ...
            if (polygon && polygon.length > 0) {
                ...
                const rectangle = Cesium.Rectangle.fromDegrees(Math.min(...lngs), Math.min(...lats), Math.max(...lngs), Math.max(...lats));
                this.viewer.camera.flyTo({
        ...
        drawRegion(polygon, name, lat, lng, subRegions) {
            ...
            const Cesium = this.Cesium;
            // 清除上一个区域
            if (this._regionEntities) {
                this._regionEntities.forEach((e) => this.viewer.entities.remove(e));
                this._regionEntities = null;
            }
        ...
        async cacuBounds(tiffId, tiffServiceCollection) {
            ...
            try {
                const res = await axios.get(`${window.config.iserverAdress}collections/${tiffServiceCollection}/items/${tiffId}.json`, {
        ...
        getBufferData3D(node) {
            ...
            const scheduleBatch = (startIndex) => {
                ...
                for (let i = startIndex; i < endIndex; i++) {
                    const item = list[i];
                    const { lat, lon } = toCesiumLonLat(item.latitude, item.longitude);
                    const circlePositions = this.computeGroundCirclePositions(lon, lat, radiusInMeters);
